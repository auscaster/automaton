...
test("buildReplayGuardPlan skips duplicate issue fingerprints", () => {
  ...
  const plan = buildReplayGuardPlan({
    ...
    operator_memory_branch: "runx/operator-memory-issue-triage-nilstate-automaton-issue-42",
  });
...
test("buildReplayGuardPlan skips duplicate PR head shas", () => {
  const plan = buildReplayGuardPlan({
    ...
    operator_memory_branch: "runx/operator-memory-issue-triage-nilstate-automaton-pr-18",
  });
